...
import {
  ArrowLeft, ArrowRight, Check, Download, FileText, Loader2, Send, X,
} from "lucide-react";
...
export function InlineDocumentGenerator({
  ...
  function applyPrefill() {
    ...
    const dt = pre.documentType ?? pre.document_type;
    const rt = pre.recipientType ?? pre.recipient_type;
    const rn = pre.recipientName ?? pre.recipient_name;
    const facts = pre.keyFacts ?? pre.key_facts ?? pre.body ?? pre.description;
    if (typeof dt === "string") setDocType(dt);
    if (typeof rt === "string") setRecipientType(rt);
    if (typeof rn === "string") setRecipientName(rn);
    if (typeof facts === "string" && facts.trim()) setKeyFactsSummary(facts.trim());
    ...
    if (typeof dt === "string") setStep("questions");
  }
  ...
  return (
    <Card className="p-5 border-accent/30">
      <div className="flex items-start justify-between gap-2 mb-4">
        <div>
          <h3 className="font-serif text-xl font-semibold">Generate a document</h3>
          <p className="text-xs text-muted-foreground mt-0.5">
            Step-by-step drafting using events and evidence from this file.
          </p>
        </div>
            ...
            return (
              <li key={s.key} className="flex-1 flex items-center gap-2 min-w-0">
                <div className="flex items-center gap-1.5 min-w-0">
                  <div className={cn(
                    "h-5 w-5 shrink-0 rounded-full border flex items-center justify-center text-[10px] font-medium",
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {DOCUMENT_TYPES.map((t) => (
            <button key={t} onClick={() => props.onPick(t)}
              className={cn(
                "group relative rounded-lg border bg-background p-3 text-left text-sm hover:border-accent transition",
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <div ref={scrollRef} className="rounded-lg border bg-background p-4 max-h-[360px] overflow-y-auto space-y-3">
...
function SelectionStep(props: {
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        <h4 className="font-serif text-lg font-semibold">Choose supporting context</h4>
        <p className="text-xs text-muted-foreground mt-0.5">
...
function ReviewStep(props: {
  ...
  const safeName = useMemo(
    () => sanitizeFilename(`${props.docType}${props.recipientName ? `-${props.recipientName}` : ""}`),
    [props.docType, props.recipientName],
  );
